        ...
        if (tangent.norm() > epsilon) // Controlla che i piani non siano paralleli
        {
            ...
            for (size_t i = 0; i < fractures.CoordVertices[id1].size() - 1; ++i)
            {
                if (Analytics::intersectrettaretta(point, tangent, fractures.CoordVertices[id1][i], fractures.CoordVertices[id1][i+1] - fractures.CoordVertices[id1][i], inter))
                {
                    ...
                    {
                        Vector3d fozzainda;
                        fozzainda= Analytics::intersectrettasemiretta(inter, tangent,fractures.CoordVertices[id2][j],fractures.CoordVertices[id2][j+1]-fractures.CoordVertices[id2][j] );
                        if (fozzainda!= limit)
                        {
                            verifica.push_back(fozzainda);
                        }
                    }
                    Vector3d fozzainda1 = Analytics::intersectrettasemiretta(inter, tangent,fractures.CoordVertices[id2][fractures.CoordVertices[id2].size()-1],fractures.CoordVertices[id2][0]-fractures.CoordVertices[id2][fractures.CoordVertices[id2].size()-1] );
                    if (fozzainda1!= limit)
                    {
                        verifica.push_back(fozzainda1);
                    }
            ...
            if (Analytics::intersectrettaretta(point, tangent, fractures.CoordVertices[id1][fractures.CoordVertices[id1].size() - 1], fractures.CoordVertices[id1][0] - fractures.CoordVertices[id1][fractures.CoordVertices[id1].size() - 1], inter))
            {
                ...
                for (size_t j = 0; j<fractures.CoordVertices[id2].size()-1;++j)
                {
                    Vector3d fozzainda = Analytics::intersectrettasemiretta(inter, tangent,fractures.CoordVertices[id2][j],fractures.CoordVertices[id2][j+1]-fractures.CoordVertices[id2][j] );
                    if (fozzainda != limit)
                    {
                        verifica.push_back(fozzainda);
                    }
            ...
            for (size_t i = 0; i < fractures.CoordVertices[id2].size() - 1; ++i)
            {
                if (Analytics::intersectrettaretta(point, tangent, fractures.CoordVertices[id2][i], fractures.CoordVertices[id2][i+1] - fractures.CoordVertices[id2][i], inter))
                {
                    ...
                    {
                        Vector3d fozzainda = Analytics::intersectrettasemiretta(inter, tangent,fractures.CoordVertices[id1][j],fractures.CoordVertices[id1][j+1]-fractures.CoordVertices[id1][j] );
                        if (fozzainda!= limit)
                        {
                            verifica.push_back(fozzainda);
                        }
                    }
                    Vector3d fozzainda1 = Analytics::intersectrettasemiretta(inter, tangent,fractures.CoordVertices[id1][fractures.CoordVertices[id1].size()-1],fractures.CoordVertices[id1][0]-fractures.CoordVertices[id1][fractures.CoordVertices[id1].size()-1] );
                    if (fozzainda1!= limit)
                    {
                        verifica.push_back(fozzainda1);
                    }
            ...
            if (Analytics::intersectrettaretta(point, tangent, fractures.CoordVertices[id2][fractures.CoordVertices[id2].size() - 1], fractures.CoordVertices[id2][0] - fractures.CoordVertices[id2][fractures.CoordVertices[id2].size() - 1], inter))
            {
                ...
                Vector3d fozzainda1 = Analytics::intersectrettasemiretta(inter, tangent,fractures.CoordVertices[id1][fractures.CoordVertices[id2].size()-1],fractures.CoordVertices[id1][0]-fractures.CoordVertices[id1][fractures.CoordVertices[id2].size()-1] );
                if (fozzainda1!= limit)
                {
                    verifica.push_back(fozzainda1);
                }
...
Vector4d calcsphere(const vector<Vector3d>& vertex_data)
{
    ...
    double max_raggio = 0;
    for (unsigned int i = 0; i < raggi.size(); ++i)
    {
                              ...
                              const Vector3d& point1, const Vector3d& dir1)
{
    ...
    if((pl-pl1).norm()<1e-6 && t>=0)
    {
        if (s >-1e-6 && s < 1.0+1e-6  )
        {
            ...
            // cout << intersec.transpose() << ";";
        }
        else
        {
            intersec = {100,100,100};
        }
    }
    else
    {
